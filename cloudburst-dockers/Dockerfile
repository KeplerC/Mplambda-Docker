...
RUN echo "rebuild9"
#Anna
RUN git clone https://github.com/keplerc/anna.git -b TopK
WORKDIR /anna
RUN git submodule init && git submodule update
RUN bash scripts/build.sh -j4 -bRelease -g

RUN git clone https://github.com/KeplerC/cluster-dropbox.git
...
RUN mkdir -p build/ && \
    cd build/ && \
    cmake -DCMAKE_BUILD_TYPE=Release .. -DCMAKE_PREFIX_PATH="~/aws;~/lambda-install" -DBUILD_SHARED_LIBS=ON && \
    make -j8 
RUN mkdir -p build/
